...
const fallbackFilters = ["All", "Adobe Commerce", "Shopify", "Migration", "Integration", "Audit", "White-Label"];
...
function Stars({ color = "var(--color-ink)", rating = 5 }: { color?: string; rating?: number }) {
...
type TestimonialsPageClientProps = {
  initialTestimonials?: TestimonialCard[];
};

export default function TestimonialsPageClient({ initialTestimonials = [] }: TestimonialsPageClientProps) {
  const [activeFilter, setActiveFilter] = useState("All");
  const [sort, setSort] = useState("recent");
  const [testimonialItems, setTestimonialItems] = useState<TestimonialCard[]>(
    initialTestimonials.length ? initialTestimonials : testimonials
  );
  ...
  useEffect(() => {
    ...
    async function loadTestimonials() {
      try {
        ...
        const data = (await response.json()) as { testimonials?: TestimonialCard[] };

        if (!cancelled && data.testimonials?.length) {
          setTestimonialItems(data.testimonials);
        }
      } catch (error) {
        console.warn("Unable to load testimonials from GraphQL.", error);
      }
    }
  ...
  const filters = useMemo(() => {
    const platformFilters = testimonialItems.map((testimonial) => testimonial.platform).filter(Boolean);
    return ["All", ...Array.from(new Set([...fallbackFilters.slice(1), ...platformFilters]))];
  }, [testimonialItems]);
  ...
  const filteredTestimonials = useMemo(() => {
      const items =
        activeFilter === "All"
        ? testimonialItems
        : testimonialItems.filter((testimonial) => testimonial.platform.includes(activeFilter));
    ...
    return items;
  }, [activeFilter, sort, testimonialItems]);
